...
## Update (July 21, 2026)
- **Authenticated User Auto-Redirect**: Updated `Login` and `Register` components (`login.ts` and `register.ts`) with `ngOnInit` checks. When an already logged-in user visits `/login` or `/register`, they are automatically redirected to their role's main page (`/customer/dashboard`, `/provider/dashboard`, or `/admin/dashboard`).
- **Customer Dashboard Component (`/customer/dashboard`)**: Built a full-featured, responsive customer workspace in `frontend/src/app/features/customer/dashboard/`. Features welcome greeting banner, active status badge, 4 stat cards (Active Bookings, Completed Hires, Saved Favourites, Account Verification), interactive category search bar with 6 category chips, active bookings tracker with inline Cancel action buttons, recommended service cards, and customer utility shortcuts.
- **Provider Dashboard Component (`/provider/dashboard`)**: Built a full-featured, responsive provider workspace in `frontend/src/app/features/provider/dashboard/`. Features welcome greeting banner, verification status badge, 4 stat cards, live incoming booking requests management with Accept/Decline action buttons, active services overview, and quick utility shortcuts.
